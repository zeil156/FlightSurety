...
contract FlightSuretyApp {
    ...
    uint8 private constant MIN_REG_AIRLINES = 4;
    uint256 public constant MAX_INS_PRICE = 1 ether;
    ...
    */
    ...
    modifier requireSenderIsRegistered() 
    {
        ...
        require(flightSuretyData.isRegistered(msg.sender), "Message sender is not a registered airline");  
        _;  // All modifiers require an "_" which indicates where the function body will be added
    }
    ...
    {
         // Modify to call sender trying to register an airline is an already registered airline
        require(flightSuretyData.isRegistered(airlineAddress), "Airline is not a registered airline so unable to register flights");  
        _;  // All modifiers require an "_" which indicates where the function body will be added
    }
    ...
    modifier requireInsuranceNotYetPurchased(address airline, string memory flightNumber, uint256 departureTime) 
    {
         // Modify to call sender trying to register an airline is an already registered airline
        require(!flightSuretyData.insPurchaseStatus(getInsuranceContractKey(airline, flightNumber, departureTime, msg.sender)), "Insurance for this flight has already been purchased");  
        _;  // All modifiers require an "_" which indicates where the function body will be added
    }
    ...
    */
    ...
    function alreadyQueued(address airlineAddress) 
                            public 
                            view 
                            returns(bool) 
    {
        return flightSuretyData.alreadyQueued(airlineAddress);  // Modify to call data contract's status
    }
   ...
   function regAirline
                            ...
                            returns(bool success)
    {
        ...
        bool isQueuedButNotYetRegistered = flightSuretyData.alreadyQueued(newAirlineAddress);
        uint256 regAirCount = flightSuretyData.regAirCount();
        uint256 numAirlineVotes = flightSuretyData.airVoteCount(newAirlineAddress);

        if(regAirCount < MIN_REG_AIRLINES){
            
            flightSuretyData.regAirline(newAirlineAddress, airlineName, true);
            ...
            if(isQueuedButNotYetRegistered){
                ...
                flightSuretyData.regAirline(newAirlineAddress, airlineName, false);
                return false;
            }
        }
    }
